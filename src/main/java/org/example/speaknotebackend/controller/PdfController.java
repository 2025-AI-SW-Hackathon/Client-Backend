...
public class PdfController {
    ...
    @PostMapping("/upload")
    public ResponseEntity<Map<String, String>> uploadForModeling(
            @RequestParam("file") MultipartFile file,
            @AuthenticationPrincipal(errorOnInvalidType = false) UserDetailsImpl user
    ) {
        if (user == null) {
            System.out.println("[PdfController] Guest user uploading file");
        } else {
            System.out.println("[PdfController] Authenticated user uploading file: id=" + user.getUserId()
